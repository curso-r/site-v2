...
donors <- fread("~/Downloads/io/Donors.csv") %>% clean_names()
```
...
Essa função também cria uma variável que indica o tempo até a próxima doação. Vamos usá-la para tirar da resposta aquelas doações que acontecem no mesmo dia ou até no dia seguinte à data que selecionamos.
...
respostas %>% 
  filter(is.na(tempo_prox_compra) | tempo_prox_compra > 1) %>% 
  group_by(t0) %>% 
...
Agora podemos voltar ao gráfico com as taxas de resposta por mês de referência, excluindo agora os ex-doadores inativos. Também combinamos anteriormente de **não considerar** pessoas que fizeram uma doação no último mês pois, a princípio não seria legal fazer uma comunicação pedindo doações para quem acabou de doar. Também vamos excluir essas pessoas no gráfico abaixo.

```{r}
respostas %>% 
  filter(is.na(tempo_prox_compra) | tempo_prox_compra > 1) %>% 
  filter(tempo_inativo <= 1000, tempo_inativo > 30) %>% 
  group_by(t0) %>% 
  summarise(taxa = mean(resposta)) %>% 
  mutate(t0 = ymd_hms(t0)) %>% 
  ggplot(aes(x = t0, y = taxa)) +
...
Vemos que dentro do período entre as linhas azul e vermelha a taxa de resposta é bem estável em torno de 10%.

Dentro deste mesmo tópico, faz sentido definirmos quais serão os meses de referência que usaremos para treinar o modelo e quais meses usaremos para validar. Como a nossa resposta incluir 1 ano para frente, temos que tomar cuidado para que a base de treino não inclua informações da base de validação.

Considere o diagrama abaixo. Cada linha nele é um mês que poderia ser usado como mês de referência. Os períodos pintados são os períodos que usamos como histórico e os em cinza escuro são os períodos que usaríamos para calcular a resposta.

Para sabermos se o nosso modelo será efetivo, temos que separar uma base de validação que não tenha intersecção de respostas com a base de treino. Por isso, estabelecemos a linha vertical vermelha. 

Veja que os meses que vamos usar para validar (Nov-16 -> Mai 17)

![](https://user-images.githubusercontent.com/4706822/47326680-40e80a80-d640-11e8-9c8d-db1d57ca902d.png)
...
Agora que já criamos a nossa variável resposta, precisamos criar as variáveis que vão nos auxiliar a prever a variável resposta. Chamamos essas variáveis de **variáveis explicativas**. 

Aqui a imaginação é livre, você pode criar qualquer variável que você pense ser relacionada com a nossa variável resposta. É incrível que com uma tabela com apenas 7 colunas (como a nossa) é possível pensar em dezenas de variáveis que podem ajudar a prever.

A única regra para criar as variáveis é que elas não podem conter nenhuma informação que tenha data > t0. Neste caso estaríamos usando informações que não teremos na hora de usar o modelo de fato.

Um pouco antes no post analisamos o `tempo_de_inatividade` que é uma variável que é fortemente relacionada com a resposta. Não podemos esquecer de colocá-la no modelo. 

Antes de começar a escrever o código, vamos pensar em outras variáveis que poderiam ser úteis:

* Quantidade de doações feitas no último ano.
* Quantidade de doações feitas nos últimos 2 anos.
* Quantidade de projetos apoiados no último ano.
* Quantidade de projetos apoiados nos últimos 2 anos.
* Proporção de doações que incluiam a doação opcional p/ DonorsChoose.
* Valor total doado no último ano
* Valor total doado nos últimos 2 anos.

Também podemos usar variáveis que estão na tabela de cadastro dos doadores (Donors.csv). 

* Se o doador é ou não professor
* O Estado em que mora o doador

Variáveis como cidade exigem cuidados na hora de usar pois possuem muitas categorias. 
Enfim, essa lista de variáveis é suficiente para construirmos o nosso primeiro modelo. Vamos agora, criar uma função que calcula essas variáveis parametrizando sempre pelo mes de referência.

Essa função é uma função da tabela de doações, da tabela de doadores e do mês de referência.

```{r}
criar_variaveis_explicativas <- function(don, donors, t0) {
  
  don <- don %>% 
    filter(donation_received_date <= t0) %>% 
    mutate(tempo_doacao = as.numeric(difftime(t0, donation_received_date, units = "days")))
  
  um_ano <- don %>% 
    filter(tempo_doacao <= 365) %>%
    select(-tempo_doacao) %>% 
    group_by(donor_id) %>% 
    summarise(
      qtd_doacoes_1 = n_distinct(donation_id),
      qtd_projetos_1 = n_distinct(project_id),
      vl_doado_1 = sum(donation_amount),
      prop_doacao_opt_1 = mean(donation_included_optional_donation == "Yes")
    )
  
  dois_anos <- don %>% 
    filter(tempo_doacao <= 730) %>% 
    select(-tempo_doacao) %>% 
    group_by(donor_id) %>% 
    summarise(
      qtd_doacoes_2 = n_distinct(donation_id),
      qtd_projetos_2 = n_distinct(project_id),
      vl_doado_2 = sum(donation_amount),
      prop_doacao_opt_2 = mean(donation_included_optional_donation == "Yes")
    )
  ...
  geral <- don %>% 
    ...
    ungroup() %>% 
    mutate(
      tempo_inatividade = difftime(t0, ultima_doacao, units = "days") %>% 
        as.integer()
    ) %>% 
    select(-ultima_doacao)
  ...
  um_ano %>% 
    full_join(dois_anos, by = "donor_id") %>% 
    full_join(geral, by = "donor_id") %>% 
    left_join(
      donors %>% select(donor_id, donor_state, donor_is_teacher),
      by = "donor_id"
    )
}
```

Agora vamos usar essa função para criar as variáveis em cada mês de referência, entre as linhas azul e vermelha no gráfico anterior.

```{r}
periodos <- seq(
  from = min(don_samp$donation_received_date) + days(1000), 
  to = max(don_samp$donation_received_date) - days(365), 
  by = "1 month"
  ) %>% 
  set_names()

explicativas <-  periodos %>% 
  map_dfr(~criar_variaveis_explicativas(don_samp, donors,  .x), .id = "t0")
```

Vamos calcular também as respostas e juntar com a base de variáveis explicativas.

```{r}
respostas <- periodos %>%  
  map_dfr(~criar_resposta(don_samp,  .x), .id = "t0")

final <- left_join(respostas, explicativas, by = c("donor_id", "t0")) %>% 
  filter(tempo_inatividade <= 1000, tempo_inatividade > 30)
```

A tabela `final` contém todas as informações que podemos usar no nosso modelo.

# Modelagem

Agora estamos prontos para ajustar nosso primeiro modelo. O primeiro passo é separar uma base de dados para validação.
